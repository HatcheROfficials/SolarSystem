...
class planet {
    ...
    shadow(){
        ...
        ctx.fillRect(this.orbit,0,this.radius*4,this.radius);
        ctx.fillRect(this.orbit,0,this.radius*4,-this.radius);
    }
...
// class defining each planet moons
class moon {
    radius; // planet radius in terms of suns radius
    orbit; // length of orbit in terms of suns radius
...
function draw() {
    ...
    ctx.beginPath();
    ctx.arc(0, 0, 2*sunR, 0, 2 * Math.PI);
    ctx.fill();
    ctx.drawImage(image,-sunR*3/2,-sunR*3/2,sunR*3,sunR*3);

    for (p of planets) {
        // drawing the planets
        ctx.save();
        ctx.rotate(((2 * Math.PI) / (1 / p.angularVel_sun)) * time.getMinutes() +
            ((2 * Math.PI) / (60 / p.angularVel_sun)) * time.getSeconds() +
        ...
        for (m of p.moons) {
            ctx.save();
            ctx.rotate(((2 * Math.PI) / (1 / m.angularVel_planet)) * time.getMinutes() +
                ((2 * Math.PI) / (60 / m.angularVel_planet)) * time.getSeconds() +
